...
// ─────────────────────────────────────────────────────────────────────────
    ...
    public WebActions getCurrentWindowHandle() {
        try {
            ...
            logger.logMessage("error", "Not able to fetched the current window handle ID, due to some windows related issue.");
        }
        return this;
    }
    ...
    public WebActions getAllAvailableWindowHandles() {
        try {
            ...
            logger.logMessage("error", "Not able to fetched the available window handle IDs, due to some windows related issue.");
        }
        return this;
    }
    ...
    public WebActions switchToFrame(int frameIndex) {
        try {
            ...
            logger.logMessage("error", "There is no frame available with index no " + frameIndex + "." + nsfe.getMessage());
        }
        return this;
    }
    ...
    public WebActions switchToFrame(String frameNameOrID) {
        try {
            if (frameNameOrID != null && !frameNameOrID.isEmpty()) {
                ...
                logger.logMessage("warn", "Not able to switch to the frame as the frame reference is either null or empty.");
            }
        } catch (NoSuchFrameException nsfe) {
    ...
    public WebActions navigateTo(String url) {
        try {
            ...
            logger.logMessage("error", "Not able to navigate to the URL as either the driver may be closed or there may be some other driver specific issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions navigateBack() {
        try {
            ...
            logger.logMessage("error", "Not able to navigate back as either the driver may be closed or there may be some other driver specific issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions navigateForward() {
        try {
            ...
            logger.logMessage("error", "Not able to navigate forward as either the driver may be closed or there may be some other driver specific issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions refreshPage() {
        try {
            ...
            logger.logMessage("error", "Not able to refresh the page as either the driver may be closed or there may be some other driver specific issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions getTheCurrentUrl() {
        try {
            ...
            logger.logMessage("error", "Not able to get the current URL as either the driver may be close or any other driver related issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions getThePageTitle() {
        try {
            ...
            logger.logMessage("error", "Not able to get the page title as either the driver may be close or any other driver related issue happened." + wde.getMessage());
        }
        return this;
    }
    ...
    public WebActions selectDropdownOptionByIndex(WebElement dropdownElement, int indexOfOption, String dropdownName) {
        try {
            if (dropdownElement != null && indexOfOption >= 0) {
                ...
                logger.logMessage("info", "Selected the " + indexOfOption + " index option from the " + dropdownName + " dropdown.");
            } else {
                logger.logMessage("warn", "Either the dropdown element provided is null or the index no is not correct.");
            }
    ...
    public WebActions selectDropdownOptionByVisibleText(WebElement dropdownElement, String visibleTextOfOption, String dropdownName) {
        try {
            if (dropdownElement != null && visibleTextOfOption != null && !visibleTextOfOption.isEmpty()) {
                ...
                logger.logMessage("info", "Selected the " + visibleTextOfOption + " option from the " + dropdownName + " dropdown.");
            } else {
                logger.logMessage("warn", "Either the dropdown element provided is null or the option's visible text provided is null/empty.");
            }
        } catch (NoSuchElementException nsee) {
            logger.logMessage("error", "The provided web element is not found on the web page or " +
                    "the option with text '" + visibleTextOfOption + "' is not available in the " + dropdownName +
                    " dropdown." + nsee.getMessage());
        }
        return this;
    }

    public WebActions selectDropdownOptionByValue(WebElement dropdownElement, String valueOfOption, String dropdownName) {
        try {
            if (dropdownElement != null && valueOfOption != null && !valueOfOption.isEmpty()) {
                select = new Select(dropdownElement);
                select.selectByValue(valueOfOption);
                logger.logMessage("info", "Selected the " + valueOfOption + " option from the " + dropdownName + " dropdown.");
            } else {
                logger.logMessage("warn", "Either the dropdown element provided is null or the option's value provided is null/empty.");
            }
        } catch (NoSuchElementException nsee) {
            logger.logMessage("error", "The provided web element is not found on the web page or " +
                    "the option with value '" + valueOfOption + "' is not available in the " + dropdownName +
                    " dropdown." + nsee.getMessage());
        }
        return this;
    }

    public WebActions deselectDropdownOptionByIndex(WebElement multiSelectDropdownElement, int indexOfOption, String dropdownName) {
        try {
            if (multiSelectDropdownElement != null && indexOfOption >= 0) {
                ...
                if (select.isMultiple()) {
                    ...
                    logger.logMessage("info", "Deselected the " + indexOfOption + " index option from the " + dropdownName + " dropdown.");
                } else {
                    logger.logMessage("warn", "The provided dropdown " + dropdownName + " is not a multi-select dropdown, so deselect operation can not be performed.");
                }
            } else {
                logger.logMessage("warn", "Either the dropdown element provided is null or the index no is not correct.");
            }
    ...
    public WebActions deselectDropdownOptionByVisibleText(WebElement multiSelectDropdownElement, String visibleTextOfOption, String dropdownName) {
        try {
            if (multiSelectDropdownElement != null && visibleTextOfOption != null && !visibleTextOfOption.isEmpty()) {
                ...
                if (select.isMultiple()) {
                    ...
                    logger.logMessage("info", "Deselected the " + visibleTextOfOption + " option from the " + dropdownName + " dropdown.");
                } else {
                    logger.logMessage("warn", "The provided dropdown " + dropdownName + " is not a multi-select dropdown, so deselect operation can not be performed.");
                }
            } else {
    ...
    public WebActions deselectDropdownOptionByValue(WebElement multiSelectDropdownElement, String valueOfOption, String dropdownName) {
        try {
            if (multiSelectDropdownElement != null && valueOfOption != null && !valueOfOption.isEmpty()) {
                select = new Select(multiSelectDropdownElement);
                if (select.isMultiple()) {
                    select.deselectByValue(valueOfOption);
                    logger.logMessage("info", "Deselected the " + valueOfOption + " option from the " + dropdownName + " dropdown.");
                } else {
                    logger.logMessage("warn", "The provided dropdown " + dropdownName + " is not a multi-select dropdown, so deselect operation can not be performed.");
                }
            } else {
    ...
    public WebActions deselectAllDropdownOptions(WebElement multiSelectDropdownElement, String dropdownName) {
        try {
            ...
            logger.logMessage("error", "The provided web element i.e., " + dropdownName + " is not found on the web page. " + nsee.getMessage());
        }
        return this;
    }
    ...
    public WebActions getTheFirstSelectedOption(WebElement dropdownElement, String dropdownName) {
        try {
            ...
            logger.logMessage("error", "The provided dropdown element is not found on the web page. " + nsee.getMessage());
        }
        return this;
    }
